...
public class SwingAnimations extends Module {
    ...
    public void renderSwordAnimation(MatrixStack matrices, float swingProgress, float equipProgress, Arm arm) {
        if (!Float.isFinite(swingProgress) || !Float.isFinite(equipProgress)) return;
        swingProgress = MathHelper.clamp(swingProgress, 0.0f, 1.0f);
        equipProgress = MathHelper.clamp(equipProgress, 0.0f, 1.0f);
        ...
        switch (mode.getValue()) {
            ...
            case "Touch" -> {
                matrices.translate(0.56F, -0.52F, -0.72F);
                float depthScale = MathHelper.clamp((float) (1 + anim * power.getValue() / 4), 0.15f, 3.5f);
                matrices.scale(1F, 1F, depthScale);
