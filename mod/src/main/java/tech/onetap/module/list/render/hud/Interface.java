...
import tech.onetap.util.math.*;
import tech.onetap.util.text.RussianUi;
...
public class Interface extends Module {
    ...
    public final ModeSetting hudStyle = new ModeSetting("Стиль HUD", "Infinyty", "Infinyty");

    private final ModeListSetting elements = new ModeListSetting("Элементы",
            new BooleanSetting("Array List", true),
    ...
    private final ModeSetting arrayListSide = new ModeSetting("Array List Side", "Right", "Right", "Left");
    private final ModeSetting arrayListColor = new ModeSetting("Array List Color", "Theme", "Theme", "Rainbow", "Custom");
    private final ColorSetting arrayListCustomColor = new ColorSetting("Array List Custom Color", 0xFFFF4D8D)
            .setVisible(() -> arrayListColor.is("Custom"));
    private final BooleanSetting arrayListBackground = new BooleanSetting("Array List Background", true);
    private final SliderSetting arrayListOpacity = new SliderSetting("Array List Opacity", 205f, 0f, 255f, 5f)
            .setVisible(arrayListBackground::getValue);
    ...
    // Kept only for loading old configs; watermark rendering and its GUI option are removed.
    private final Draggable watermarkDrag = DragManager.installDrag(this, "WatermarkLegacy", 4, 4);
    ...
    public void onEventHUD(EventHUD e) {
        ...
        renderInfinytyWatermark(e.getDrawContext());
        ...
        if (elements.isEnabled("Array List")) {
            renderArrayList(e.getDrawContext());
        }
    ...
    private void renderInfinytyWatermark(DrawContext context) {
        String text = "Infinyty  |  " + mc.getCurrentFps() + " FPS";
        float x = watermarkDrag.getX();
        float y = watermarkDrag.getY();
        float width = Fonts.SFMEDIUM.get().getWidth(text, 8f) + 12f;
        drawBackground(x, y, width, 16f, 5f, 235);
        DrawUtil.drawRound(x + 4f, y + 4f, 2f, 8f, 1f, ColorProvider.getThemeColor());
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), text, x + 9f, y + 4f,
                ColorProvider.rgba(245, 245, 248, 255), 8f);
        watermarkDrag.setWidth(width);
        watermarkDrag.setHeight(16f);
    }

    private void renderArrayList(DrawContext context) {
        if (mc.player == null) return;

        List<Module> enabled = Onetap.getInstance().getModuleStorage().getModules().stream()
                .filter(Module::isEnabled)
                .filter(module -> module != this)
                .sorted((left, right) -> Float.compare(
                        Fonts.SFMEDIUM.get().getWidth(RussianUi.module(right.getName()), 7.5f),
                        Fonts.SFMEDIUM.get().getWidth(RussianUi.module(left.getName()), 7.5f)))
                .toList();
        ...
        for (Module module : enabled) {
            String name = RussianUi.module(module.getName());
            float textWidth = Fonts.SFMEDIUM.get().getWidth(name, 7.5f);
            float width = textWidth + 13f;
            boolean left = arrayListSide.is("Left");
            float x = left ? 5f : screenWidth - width - 5f;
            float hue = enabled.size() <= 1 ? 0f : index / (float) (enabled.size() - 1);
            int accent;
            if (arrayListColor.is("Rainbow")) {
                float rainbowHue = ((System.currentTimeMillis() / 12L) + index * 35L) % 360L / 360f;
                accent = java.awt.Color.HSBtoRGB(rainbowHue, 0.72f, 1f);
            } else if (arrayListColor.is("Custom")) {
                accent = arrayListCustomColor.getValue();
            } else {
                accent = ColorProvider.interpolateColor(
                        ColorProvider.getThemeColor(), ColorProvider.getThemeColorTwo(), hue);
            }

            if (arrayListBackground.getValue()) {
                int opacity = arrayListOpacity.getIntValue();
                DrawUtil.drawRoundBlur(x, y, width, 13f, 3.5f,
                        ColorProvider.rgba(0, 0, 0, Math.min(170, opacity)), 9f);
                DrawUtil.drawRound(x, y, width, 13f, 3.5f,
                        ColorProvider.rgba(12, 14, 18, opacity));
            }
            DrawUtil.drawRound(left ? x + 0.5f : x + width - 2f, y + 2f, 1.5f, 9f, 1f,
                    ColorProvider.setAlpha(accent, 255));
            DrawUtil.drawText(Fonts.SFMEDIUM.get(), name, x + (left ? 6f : 5f), y + 3f,
                    ColorProvider.rgba(240, 242, 248, 255), 7.5f);
            y += 15f;
            index++;
        }
    }

    private void renderTargetHUDNova(DrawContext context) {
        ...
        if (animation.getValue() <= 0.04f || !(lastTarget instanceof LivingEntity living)) return;
        ...
        int themeTwo = ColorProvider.getThemeColorTwo();

        DrawUtil.drawRoundBlur(x, y, width, height, 7f, ColorProvider.rgba(0, 0, 0, (int) (155 * anim)), 16f);
        DrawUtil.drawRound(x, y, width, height, 7f, ColorProvider.rgba(12, 14, 19, (int) (232 * anim)));
        DrawUtil.drawRound(x, y, 2.5f, height, 2f, ColorProvider.setAlpha(theme, alpha));

        float headX = x + 7f;
        float headY = y + 6f;
        float headSize = 31f;
        int hurtTint = ColorProvider.rgba(255, (int) (255 * (1f - living.hurtTime / 10f)),
                (int) (255 * (1f - living.hurtTime / 10f)), alpha);

        if (living instanceof AbstractClientPlayerEntity player) {
            try {
                int texture = mc.getTextureManager().getTexture(player.getSkinTextures().texture()).getGlId();
                Builder.texture()
                        .size(new SizeState(headSize, headSize))
                        .radius(new QuadRadiusState(6f))
                        .color(new QuadColorState(hurtTint))
                        .texture(8f / 64f, 8f / 64f, 8f / 64f, 8f / 64f, texture)
                        .smoothness(1f).build()
                        .render(context.getMatrices().peek().getPositionMatrix(), headX, headY);
            } catch (Exception ignored) {
                DrawUtil.drawRound(headX, headY, headSize, headSize, 6f, ColorProvider.rgba(35, 38, 48, alpha));
            }
        } else {
            DrawUtil.drawRound(headX, headY, headSize, headSize, 6f, ColorProvider.rgba(35, 38, 48, alpha));
            String initial = living.getName().getString().isEmpty() ? "?" : living.getName().getString().substring(0, 1).toUpperCase();
            DrawUtil.drawText(Fonts.SFBOLD.get(), initial, headX + 10f, headY + 8f,
                    ColorProvider.setAlpha(theme, alpha), 14f);
        }

        float textX = headX + headSize + 7f;
        String name = living.getName().getString();
        DrawUtil.drawText(Fonts.SFBOLD.get(), name, textX, y + 6f,
                ColorProvider.rgba(245, 247, 255, alpha), 8.5f, 0.25f, 0.7f, width - 50f);

        float health = Math.max(0f, living.getHealth());
        float absorption = Math.max(0f, living.getAbsorptionAmount());
        float maxHealth = Math.max(1f, living.getMaxHealth());
        float distance = mc.player == null ? 0f : mc.player.distanceTo(living);
        String info = String.format(java.util.Locale.US, "%.1f HP  •  %.1fm", health + absorption, distance);
        DrawUtil.drawText(Fonts.SFREGULAR.get(), info, textX, y + 18f,
                ColorProvider.rgba(166, 173, 190, alpha), 6.8f);

        float barX = textX;
        float barY = y + 31.5f;
        float barWidth = width - (textX - x) - 7f;
        float barHeight = 4.5f;
        hpAnimation.run(barWidth * MathHelper.clamp((health + absorption) / (maxHealth + absorption), 0f, 1f));
        float fill = MathHelper.clamp((float) hpAnimation.getValue(), 0f, barWidth);
        DrawUtil.drawRound(barX, barY, barWidth, barHeight, 2.2f, ColorProvider.rgba(34, 38, 48, alpha));
        if (fill > 0.2f) {
            int[] colors = ColorProvider.getOrbitalRect(theme, themeTwo, 300.0, alpha);
            DrawUtil.drawRound(barX, barY, fill, barHeight, 2.2f, colors[0], colors[1], colors[2], colors[3]);
        }
    ...
    private void onPacket(EventPacket e) {
        if (mc.world != null && e.getPacket() instanceof EntityStatusS2CPacket status && status.getStatus() == 9) {
            Entity entity = status.getEntity(mc.world);
            if (entity instanceof PlayerEntity player) {
                ItemStack used = player.getActiveItem().isEmpty() ? player.getOffHandStack() : player.getActiveItem();
                if (used.isEmpty()) used = player.getMainHandStack();
                if (!used.isEmpty()) recentUses.put(player.getId(), new RecentUse(used.getName().getString(), System.currentTimeMillis()));
            }
        }
    ...
    /** Строка списка Keybinds: подпись, текст клавиши и прозрачность (для анимации модулей). */
    private record BindRow(String label, String key, float alpha) {}

    /** Собирает биндов модулей + биндов настроек в формате "Модуль Настройка: значение". */
    private List<BindRow> collectBindRows() {
        boolean instant = hudStyle.is("Infinyty");
        ...
        for (Module m : Onetap.getInstance().getModuleStorage().getModules()) {
            ...
            for (Setting s : m.getSettings()) {
                if (s.isBound()) {
                    int settingKey = s instanceof tech.onetap.module.settings.ActionBindSetting action
                            ? action.getKeyCode() : s.getKey();
                    rows.add(new BindRow(m.getName() + " " + s.getName() + ": " + s.getBindDisplayValue(),
                            KeyStorage.getKey(settingKey), 1f));
                }
            }
        }
        return rows;
    ...
    private void renderKeyBindsOld(DrawContext context) {
        List<BindRow> activeRows = collectBindRows();
        if (activeRows.isEmpty() && !(mc.currentScreen instanceof ChatScreen)) return;
        ...
        if (activeRows.isEmpty() && mc.currentScreen instanceof ChatScreen) {
        ...
        keyBindsDrag.setHeight(posY - keyBindsDrag.getY());
    }
    ...
    private void renderKeyBindsCelestial(DrawContext context) {
        ...
        alpha.run((activeRows.isEmpty() && !chatOpen) ? 0f : 1f);
    ...
    private void renderKeyBinds(DrawContext context) {
        ...
        if (hudStyle.is("Celestial")) {
            ...
            renderKeyBindsOld(context);
        } else if (hudStyle.is("Infinyty")) {
            renderKeyBindsCatlavan(context);
        }
    ...
    private void renderKeyBindsClassic(DrawContext context) {
        ...
        List<BindRow> allRows = collectBindRows();

        boolean isFound = !allRows.isEmpty();
        if (isFound) alpha.run(1);
        else if (!(mc.currentScreen instanceof ChatScreen)) alpha.run(0);
    ...
    private void renderStaffList(DrawContext context) {
        ...
        if (hudStyle.is("Celestial")) {
            ...
            renderStaffListOld(context);
        } else if (hudStyle.is("Infinyty")) {
            renderStaffListCatlavan(context);
        } else {
            renderStaffListClassic(context);
        }
    }

    private void renderStaffListOld(DrawContext context) {
        final boolean chatOpen = mc.currentScreen instanceof ChatScreen;
        ...
        for (Staff staff : activeStaff) {
            ...
            if (staff.status == Status.VANISHED || staff.isSpec) {
                nameColor = 0xFF5050;
            } else if (mc.world != null && mc.world.getPlayers().stream()
            ...
            context.drawText(mc.textRenderer, fullRowText, 0, 0, -1, true);

            context.getMatrices().pop();

            float rowWidth = mc.textRenderer.getWidth(fullRowText) * scale;
            if (rowWidth > maxWidth) maxWidth = rowWidth;
        ...
        staffListDrag.setHeight(posY - startY);
    }
    ...
    private void renderStaffListCelestial(DrawContext context) {
        ...
        for (Staff staff : activeStaff) {
            ...
            if (itemA >= 4) {
                ...
                int nameColor;
                if (staff.status == Status.VANISHED || staff.isSpec) nameColor = ColorProvider.rgba(255, 50, 50, itemA);
                else if (mc.world != null && mc.world.getPlayers().stream()
    ...
    private void renderPotions(DrawContext context) {
        ...
        if (hudStyle.is("Celestial")) {
            ...
            renderPotionsOld(context); // Наш новый режим
        } else if (hudStyle.is("Infinyty")) {
            renderPotionsCatlavan(context);
        } else {
    ...
    private void renderPotionsOld(DrawContext context) {
        ...
        if (visible.isEmpty() && !(mc.currentScreen instanceof ChatScreen)) return;
        ...
        potionsDrag.setWidth(maxWidth);
        potionsDrag.setHeight(posY - potionsDrag.getY());
    }
    ...
    private void drawCatlavanPanel(float x, float y, float w, float h, int alpha) {
        DrawUtil.drawRound(x, y, w, h, 3f, ColorProvider.rgba(15, 15, 16, alpha));
        DrawUtil.drawRound(x + 0.5f, y + 0.5f, w - 1f, h - 1f, 2.5f, ColorProvider.rgba(255, 255, 255, (int) (alpha * 0.04f)));
        DrawUtil.drawRound(x, y + 3f, 1.5f, h - 6f, 1f, ColorProvider.setAlpha(ColorProvider.getThemeColor(), alpha));
    }

    private void renderWatermarkCatlavan(DrawContext context) {
        float x = watermarkDrag.getX();
        float y = watermarkDrag.getY();
        ...
        float pad = 5f;
        float logoSize = 10f;
        String info = String.format("%s | %d fps | %d ms | %s", name, fps, ping, coords);
        float line1 = logoSize + 4f + bold.getWidth("CATLAVAN", 10f) + 2f + bold.getWidth("1.9", 7f);
        float line2 = regular.getWidth(info, 7f) + pad;
        float w = Math.max(line1 + pad * 2f, line2);
        float h = pad * 2f + logoSize + 3f + 9f;
        int a = 230;

        drawCatlavanPanel(x, y, w, h, a);

        DrawUtil.drawRound(x + pad, y + pad, logoSize, logoSize, 2f, ColorProvider.getThemeColor());
        DrawUtil.drawText(bold, "C", x + pad + 2.6f, y + pad + 1f, ColorProvider.rgba(10, 10, 10, a), 7f);

        float tx = x + pad + logoSize + 4f;
        DrawUtil.drawText(bold, "CATLAVAN", tx, y + pad - 1f, ColorProvider.rgba(255, 255, 255, a), 10f);
        float vx = tx + bold.getWidth("CATLAVAN", 10f) + 2f;
        DrawUtil.drawText(bold, "1.9", vx, y + pad + 0.5f, ColorProvider.setAlpha(ColorProvider.getThemeColor(), a), 7f);

        DrawUtil.drawText(regular, info, x + pad, y + pad + logoSize + 3f, ColorProvider.rgba(163, 163, 166, a), 7f);

        watermarkDrag.setWidth(w);
        watermarkDrag.setHeight(h);
    }

    private void renderKeyBindsCatlavan(DrawContext context) {
        List<BindRow> rows = collectBindRows();
        if (rows.isEmpty() && !(mc.currentScreen instanceof ChatScreen)) return;
        ...
        float maxW = 90f;
        for (BindRow r : rows) {
            float rw = bold.getWidth(r.label(), 7f) + font.getWidth("[" + r.key() + "]", 7f) + 16f;
            maxW = Math.max(maxW, rw);
        }
        float w = maxW;
        float h = headerH + rows.size() * rowH + (rows.isEmpty() ? rowH : 3f);
        int a = 230;

        drawCatlavanPanel(x, y, w, h, a);
        DrawUtil.drawText(bold, "КЛАВИШИ", x + 5f, y + 3.5f, ColorProvider.rgba(163, 163, 166, a), 6.5f);
        DrawUtil.drawRound(x + 5f, y + 13f, w - 10f, 0.5f, 1f, ColorProvider.rgba(255, 255, 255, (int) (a * 0.07f)));

        if (rows.isEmpty()) {
            DrawUtil.drawText(font, "Нет активных биндов", x + 5f, y + headerH + 1f, ColorProvider.rgba(93, 93, 97, a), 7f);
        } else {
            float curY = y + headerH + 1f;
            for (BindRow r : rows) {
                DrawUtil.drawText(bold, r.label(), x + 5f, curY, ColorProvider.rgba(255, 255, 255, a), 7f);
                String key = "[" + r.key() + "]";
                DrawUtil.drawText(font, key, x + w - font.getWidth(key, 7f) - 5f, curY, ColorProvider.setAlpha(ColorProvider.getThemeColor(), a), 7f);
                curY += rowH;
            }
        }
    ...
    private void renderStaffListCatlavan(DrawContext context) {
        List<Staff> active = staffPlayers.stream().filter(s -> s.isOnServer).toList();
        if (active.isEmpty() && !(mc.currentScreen instanceof ChatScreen)) return;
        ...
        float maxW = 70f;
        for (Staff s : active) {
            float rw = bold.getWidth(s.prefix.getString(), 7f) + 30f;
            maxW = Math.max(maxW, rw);
        }
        float w = maxW;
        float h = headerH + active.size() * rowH + (active.isEmpty() ? rowH : 3f);
        int a = 230;

        drawCatlavanPanel(x, y, w, h, a);
        DrawUtil.drawText(bold, "СТАФФ", x + 5f, y + 3.5f, ColorProvider.rgba(163, 163, 166, a), 6.5f);
        DrawUtil.drawRound(x + 5f, y + 13f, w - 10f, 0.5f, 1f, ColorProvider.rgba(255, 255, 255, (int) (a * 0.07f)));

        if (active.isEmpty()) {
            DrawUtil.drawText(font, "Нет активных модераторов", x + 5f, y + headerH + 1f, ColorProvider.rgba(93, 93, 97, a), 7f);
        } else {
            float curY = y + headerH + 1f;
            for (Staff s : active) {
                DrawUtil.drawText(bold, s.prefix.getString(), x + 5f, curY, ColorProvider.rgba(235, 235, 235, a), 7f);
                if (s.isSpec || s.status == Status.VANISHED) {
                    String tag = "[" + Status.VANISHED.string + "]";
                    DrawUtil.drawText(font, tag, x + w - font.getWidth(tag, 7f) - 5f, curY, Status.VANISHED.color, 7f);
                }
                curY += rowH;
            }
        }

        staffListDrag.setWidth(w);
        staffListDrag.setHeight(h);
    }

    private void renderPotionsCatlavan(DrawContext context) {
        List<PotionItem> visible = potionItems.stream()
                .filter(pi -> pi.active)
                .sorted(Comparator.comparing(pi -> pi.name))
                .toList();
        if (visible.isEmpty() && !(mc.currentScreen instanceof ChatScreen)) return;
        ...
        float w = maxW;
        float h = headerH + visible.size() * rowH + (visible.isEmpty() ? rowH : 3f);
        int a = 230;

        drawCatlavanPanel(x, y, w, h, a);
        DrawUtil.drawText(bold, "БАФЫ", x + 5f, y + 3.5f, ColorProvider.rgba(163, 163, 166, a), 6.5f);
        DrawUtil.drawRound(x + 5f, y + 13f, w - 10f, 0.5f, 1f, ColorProvider.rgba(255, 255, 255, (int) (a * 0.07f)));

        if (visible.isEmpty()) {
            DrawUtil.drawText(font, "Нет активных бафов", x + 5f, y + headerH + 1f, ColorProvider.rgba(93, 93, 97, a), 7f);
        } else {
            float curY = y + headerH + 1f;
            for (PotionItem p : visible) {
                int totalSec = Math.max(0, p.durationTicks / 20);
        ...
        potionsDrag.setWidth(w);
        potionsDrag.setHeight(h);
    }
    public void update() {
    ...
    private void renderWatermark(DrawContext context) {
        ...
        if (hudStyle.is("Celestial")) {
            ...
            renderWatermarkOld(context);
        } else if (hudStyle.is("Infinyty")) {
            renderWatermarkCatlavan(context);
        } else {
            renderWatermarkNursultan(context);
        }
    }

    private void renderWatermarkOld(DrawContext context) {
        float posX = watermarkDrag.getX();
        float posY = watermarkDrag.getY();
        float fontSize = 7.5f;
    ...
    private void renderWatermarkCelestial(DrawContext context) {
        ...
        String fullText = "LayF infinyty | " + userText + " | " + uid + " | " + timeText;
        float fontSize = 7.5f;
    ...
    private MsdfFont resolveFont(String mode) {
        return switch (mode) {
            case "SemiBold" -> Fonts.SFSEMIBOLD.get();
            case "Medium" -> Fonts.SFMEDIUM.get();
            case "Regular" -> Fonts.SFREGULAR.get();
            case "Rubik" -> Fonts.RUBIK.get();
            default -> Fonts.SFBOLD.get();
        };
    }
    ...
    private void renderMACEDMG(DrawContext context) {
        ...
        // 1. Проверяем наличие булавы в инвентаре игрока
        ...
        // 2. Получаем текущую дистанцию падения
        float fallDistance = mc.player.fallDistance;

        // Базовый урон булавы при обычном ударе на земле (6 HP)
        double totalDamage = 0.0;
        boolean canSmash = fallDistance >= 1.5f;

        // Удар в падении (Smash attack) активируется при падении с 1.5 блоков и более
        if (canSmash) {
            double heightDamage = 0.0;
            float blocksFallen = fallDistance;

            // Расчет чистого урона по стадиям высоты:
            // Первые 3 блока (+4 HP за каждый блок)
            if (blocksFallen > 3.0f) {
                heightDamage += 3.0f * 4.0;
                blocksFallen -= 3.0f;

                // Следующие 5 блоков (+2 HP за каждый блок)
                if (blocksFallen > 5.0f) {
                    heightDamage += 5.0f * 2.0;
                    blocksFallen -= 5.0f;

                    // Все последующие блоки падения (+1 HP за каждый блок)
            ...
            // Суммируем базу и урон от высоты, затем умножаем на 1.5 (критический удар)
            totalDamage = (totalDamage + heightDamage) * 1.5;
        }

        // 3. Определение цвета текста в зависимости от условий
        int textColor;
        if (!canSmash) {
            textColor = -1; // Белый цвет (игрок на земле или падает меньше 1.5 блоков)
        } else if (totalDamage < 10.0) {
            textColor = 0xFF55FF55; // Светло-зеленый
        } else if (totalDamage < 50.0) {
            textColor = 0xFFFFFF55; // Желтый
        } else if (totalDamage < 100.0) {
            textColor = 0xFFFFAA00; // Оранжевый
        } else {
            textColor = 0xFFFF5555; // Светло-красный (100 HP и больше)
        }

        // 4. Форматирование текста (выводим чистый урон, например: "18.0 DMG")
        String text = String.format(java.util.Locale.US, "%.1f DMG", totalDamage);
        float textWidth = Fonts.SFREGULAR.get().getWidth(text, 9);
    ...
    private void renderTargetHUDCelestial(DrawContext context) {
        ...
        if (target != null) {
            lastTarget = target;
            animation.run(1);
            armorAnim.run(1); // Анимация брони
        } else {
            animation.run(0);
            armorAnim.run(0);
        }
        ...
        // ОРИГИНАЛЬНЫЕ РАЗМЕРЫ: Высота 37, как в исходном коде
        ...
        DrawUtil.drawText(Fonts.SFBOLD.get(), name, textX, y + 5, ColorProvider.rgba(230, 230, 230, alphaInt), 9f, 0.3f, 0.7f, width);

        // Замените старое получение здоровья на это:
        ...
        // РЕНДЕР БРОНИ НАД ХУДОМ (NURSULTAN STYLE)
        float armorAlpha = (float) armorAnim.getValue();
        if (armorAlpha > 0.05f) {
            List<ItemStack> items = new ArrayList<>();
            // Стандартный порядок Nursultan: Armor -> Offhand -> Mainhand
            ...
            if (activeItemsCount > 0) {
                ...
                // Рендерим ровно НАД худом (y - высота_слота - отступ)
    ...
    private void renderTargetHUDOld(DrawContext context) {
        ...
        float anim = (float) animation.getValue();
        if (anim <= 0.04f || !(lastTarget instanceof LivingEntity living)) return;
        ...
        // ── Масштаб-анимация от центра (x+36, y+26) ──
        context.getMatrices().push();
        context.getMatrices().translate(x + 36f, y + 26f, 0f);
        context.getMatrices().scale(anim, anim, 1f);
        context.getMatrices().translate(-x - 36f, -y - 26f, 0f);

        // ── Блюр + тень позади карточки ──
        DrawUtil.drawRoundBlur(x, y, width, height, 12f, ColorProvider.rgba(255, 255, 255, (int) (255 * anim)), 5f);
        DrawUtil.drawRoundBlur(x, y, width, height, 12f, ColorProvider.rgba(0, 0, 0, (int) (200 * anim)), 18f);

        // ── Белая карточка ──
        DrawUtil.drawRound(x, y, width, height, 5f, ColorProvider.rgba(255, 255, 255, (int) (255 * anim)));

        // ── Голова 30×30 с hurt-тинтом и чёрной обводкой ──
        float headSize = 30f;
        float headX = x + 5f;
        float headY = y + 5f;
        DrawUtil.drawRound(headX - 0.8f, headY - 0.8f, headSize + 1.6f, headSize + 1.6f, 2f,
                ColorProvider.rgba(0, 0, 0, alpha));
        int hurtTint = ColorProvider.rgba(255, (int) (255 * (1f - living.hurtTime / 10f)),
                (int) (255 * (1f - living.hurtTime / 10f)), alpha);
        ...
        if (living instanceof AbstractClientPlayerEntity player) {
            try {
                ...
                DrawUtil.drawRound(headX, headY, headSize, headSize, 2f, ColorProvider.rgba(210, 212, 220, alpha));
            }
        } else {
            DrawUtil.drawRound(headX, headY, headSize, headSize, 2f, ColorProvider.rgba(210, 212, 220, alpha));
            String initial = living.getName().getString().isEmpty() ? "?" : living.getName().getString().substring(0, 1).toUpperCase();
            DrawUtil.drawText(Fonts.SFBOLD.get(), initial, headX + 10f, headY + 9f,
                    ColorProvider.setAlpha(theme, alpha), 14f);
        }

        // ── Броня (шлем → ботинки слева направо) + оффхенд ──
        ItemStack[] armorSlots = new ItemStack[4];
        int armorIndex = 0;
        for (ItemStack stack : living.getArmorItems()) {
            if (armorIndex < 4) armorSlots[armorIndex++] = stack;
        }
        float iconX = x + 40f;
        float iconY = y + 2f;
        for (int i = 3; i >= 0; i--) {
            ...
            if (stack != null && !stack.isEmpty()) {
                context.getMatrices().push();
                context.getMatrices().translate(iconX, iconY, 0f);
                context.drawItem(stack, 0, 0);
                context.getMatrices().pop();
                if (stack.isDamageable() && stack.getMaxDamage() > 0) {
                    String durability = String.valueOf(stack.getMaxDamage() - stack.getDamage());
                    float durWidth = Fonts.SFMEDIUM.get().getWidth(durability, 4.5f);
                    DrawUtil.drawText(Fonts.SFMEDIUM.get(), durability, iconX + 8f - durWidth / 2f, iconY + 12f,
                            ColorProvider.rgba(0, 0, 0, alpha), 4.5f);
                }
            }
            iconX += 18f;
        }
        ...
        context.getMatrices().pop();
        if (!offhand.isEmpty()) {
            String offhandName = offhand.getItem() == Items.TOTEM_OF_UNDYING
                    ? "Талик"
                    : offhand.getName().getString();
            DrawUtil.drawText(Fonts.SFMEDIUM.get(), offhandName, iconX + 18f, iconY + 4f,
                    ColorProvider.rgba(0, 0, 0, alpha), 5f);
        }

        // ── Имя ──
        String name = living.getName().getString();
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), name, x + 40f, y + 34f,
                ColorProvider.rgba(22, 22, 28, alpha), 6.5f, 0.25f, 0.7f, 62f);

        // ── Здоровье ──
        float health = Math.max(0f, living.getHealth());
        float maxHealth = Math.max(1f, living.getMaxHealth());
        float healthWid = MathHelper.clamp(health / maxHealth, 0f, 1f) * 80f;
        hpAnimation.run(healthWid);
        float healthBarWidth = MathHelper.clamp((float) hpAnimation.getValue(), 0f, 80f);

        String healthText = String.format(java.util.Locale.US, "%.1f", health);
        if (health >= 3f) {
            DrawUtil.drawText(Fonts.SFMEDIUM.get(), healthText, x + 105f, y + 34f,
                    ColorProvider.rgba(20, 20, 26, alpha), 6f);
        }
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), "HP", x + 121f, y + 33.5f,
                ColorProvider.rgba(255, 255, 255, alpha), 6.5f);

        // ── Полоса здоровья с свечением ──
        float barX = x + 38f;
        float barY = y + 40f;
        DrawUtil.drawRoundBlur(barX, barY, 17f + healthBarWidth, 4.5f, 8f,
                ColorProvider.rgba(0, 81, 255, (int) (255 * anim)), 6f);
        DrawUtil.drawRound(barX, barY, 17f + healthBarWidth, 5f, 2.5f,
                ColorProvider.setAlpha(theme, alpha));

        // ── Золотое яблоко (absorption) ──
        StatusEffectInstance absorption = living.getStatusEffect(StatusEffects.ABSORPTION);
        if (absorption != null) {
            boolean ateEnchanted = absorption.getAmplifier() >= 3;
            ItemStack gapple = ateEnchanted ? new ItemStack(Items.ENCHANTED_GOLDEN_APPLE) : new ItemStack(Items.GOLDEN_APPLE);
            context.getMatrices().push();
            context.getMatrices().translate(x + 40f, y + 50f, 0f);
            context.getMatrices().scale(0.75f, 0.75f, 1f);
            context.drawItem(gapple, 0, 0);
            context.getMatrices().pop();
            DrawUtil.drawText(Fonts.SFREGULAR.get(), ateEnchanted ? "Съел чарку" : "Съел яблоко", x + 57f, y + 53f,
                    ColorProvider.rgba(0, 0, 0, alpha), 5f);
        }

        // ── Частицы при получении урона ──
        if (living.hurtTime == 8) {
            particles.add(new TargetHudParticle(
                    x + 20f,
                    y + 15f,
                    (float) ((Math.random() - 0.5) * 3.8f),
                    (float) ((Math.random() - 0.5) * 2.8f),
                    theme));
        }
        particles.removeIf(p -> p.life <= 0f);
        for (TargetHudParticle p : particles) {
            p.tick();
            DrawUtil.drawRound(p.x, p.y, 2.2f, 2.2f, 1.1f,
                    ColorProvider.setAlpha(p.color, (int) (alpha * p.life)));
        }
    ...
    private static class TargetHudParticle {
        float x, y, velX, velY;
        float life = 1f;
        final int color;

        TargetHudParticle(float x, float y, float velX, float velY, int color) {
            this.x = x;
            this.y = y;
            this.velX = velX;
            this.velY = velY;
    ...
    private void renderTargetHUDMini(DrawContext context) {
        ...
        // === ГЕОМЕТРИЯ ХУДА ===
        // Без головы худ стал намного компактнее (Nursultan Mini)
        float width = 90f;
        float height = 28f;
        float x = targetHUDDrag.getX();
        float y = targetHUDDrag.getY();

        // Фон в стиле Nursultan
        drawBackground(x, y, width, height, 4f, alphaInt);
        ...
        float currentHp = Math.max(0, livingEntity.getHealth());
        ScoreboardHealth sbh = Onetap.getInstance().getModuleStorage().get(ScoreboardHealth.class);
        if (sbh != null && sbh.isEnabled() && lastTarget instanceof AbstractClientPlayerEntity playerEntity) {
            float scoreboardHp = sbh.getRealHp(playerEntity);
            if (scoreboardHp != -1) {
                currentHp = scoreboardHp;
            }
        ...
        float barHeight = 3.5f; // Фиксированная толщина 2.5
        ...
        // Отрисовка подложки (задника) полосы
        ...
        // Отрисовка основной полосы здоровья
        if (hpWNow > 0.5f) {
            DrawUtil.drawRound(barX, barY, hpWNow, barHeight, 1f, hpLeft, hpLeft, hpRight, hpRight);
        }

        // Отрисовка полосы AB поверх левого края простой полосы здоровья
        if (absHp > 0.05f) {
            ...
            if (abWNow > 0.5f) {
                ...
                // Рисуется ровно поверх левого края основной полосы
                DrawUtil.drawRound(barX, barY, abWNow, barHeight, 1f, absLeftColor, absLeftColor, absRightColor, absRightColor);
            }
        }

        // === ТЕКСТОВЫЕ ПОКАЗАТЕЛИ (ХП над концом полосы, AB зафиксировано слева над ХП) ===
        String hpText = String.format(java.util.Locale.US, "%.1f", animatedHp);
        float hpTextWidth = Fonts.SFBOLD.get().getWidth(hpText, 6.5f);

        // Текущее ХП динамически следует над концом заполненной полосы
        float hpTextX = barX + hpWNow - (hpTextWidth / 2f);
        hpTextX = MathHelper.clamp(hpTextX, barX, barX + barWidth - hpTextWidth); // зажим в границах
        float hpTextY = barY - 7.5f;

        // Полоса/Текст AB находится СЛЕВА над полосой здоровья зафиксировано
        float absTextX = barX;
        ...
        if (absHp > 0.05f) {
            ...
            // Защита от наслоения текста ХП на зафиксированный текст AB при низком здоровье:
            // Если позиция текста ХП пытается уйти левее конца текста AB, сдвигаем текст ХП чуть "сзади" (правее)
            float minAllowedHpX = absTextX + absTextWidth + 3f; // 3 пикселя зазора
            if (hpTextX < minAllowedHpX) {
                hpTextX = minAllowedHpX;
            }

            // Отрисовка зафиксированного текста AB
            DrawUtil.drawText(Fonts.SFBOLD.get(), absText, absTextX, absTextY, ColorProvider.rgba(255, 215, 0, alphaInt), 6.5f);
        }

        // Отрисовка динамического текста ХП
        DrawUtil.drawText(Fonts.SFBOLD.get(), hpText, hpTextX, hpTextY, ColorProvider.rgba(225, 225, 225, alphaInt), 6.5f);

        // Обновление размеров драг-зоны
        targetHUDDrag.setWidth(width);
        targetHUDDrag.setHeight(height);
    }

    private void renderTargetHUDCatlavan(DrawContext context) {
        ...
        float anim = MathHelper.clamp((float) animation.getValue(), 0f, 1f);
        if (anim <= 0.04f || !(lastTarget instanceof LivingEntity living)) return;
        ...
        context.getMatrices().push();
        context.getMatrices().translate(x + 30f, y + 24f, 0f);
        context.getMatrices().scale(anim, anim, 1f);
        context.getMatrices().translate(-x - 30f, -y - 24f, 0f);

        drawCatlavanPanel(x, y, width, height, alpha);

        // ── Голова 26×26 с чёрной обводкой и хёрт-тинтом ──
        float headSize = 26f;
        float headX = x + 5f;
        float headY = y + 5f;
        DrawUtil.drawRound(headX - 0.8f, headY - 0.8f, headSize + 1.6f, headSize + 1.6f, 2f,
        ...
        if (living instanceof AbstractClientPlayerEntity player) {
            ...
            DrawUtil.drawRound(headX, headY, headSize, headSize, 2f, ColorProvider.rgba(210, 212, 220, alpha));
            String initial = living.getName().getString().isEmpty() ? "?" : living.getName().getString().substring(0, 1).toUpperCase();
            DrawUtil.drawText(Fonts.SFBOLD.get(), initial, headX + 9f, headY + 8f,
                    ColorProvider.setAlpha(theme, alpha), 12f);
        }

        // ── Имя + пинг ──
        String name = living.getName().getString();
        int ping = 0;
        if (living instanceof AbstractClientPlayerEntity p && mc.getNetworkHandler() != null) {
            PlayerListEntry entry = mc.getNetworkHandler().getPlayerListEntry(p.getUuid());
            if (entry != null) ping = entry.getLatency();
        }
        float nameX = x + 37f;
        DrawUtil.drawText(Fonts.SFBOLD.get(), name, nameX, y + 5f, ColorProvider.rgba(255, 255, 255, alpha), 7.5f);
        String pingText = ping + " ms";
        DrawUtil.drawText(Fonts.SFREGULAR.get(), pingText, x + width - 5f - Fonts.SFREGULAR.get().getWidth(pingText, 6f),
                y + 6f, ColorProvider.rgba(163, 163, 166, alpha), 6f);

        // ── Полоса здоровья ──
        float health = Math.max(0f, living.getHealth());
        float maxHealth = Math.max(1f, living.getMaxHealth());
        float barW = width - 42f;
        float healthWid = MathHelper.clamp(health / maxHealth, 0f, 1f) * barW;
        hpAnimation.run(healthWid);
        float hpW = MathHelper.clamp((float) hpAnimation.getValue(), 0f, barW);
        float barX = x + 37f;
        float barY = y + 17f;
        DrawUtil.drawRound(barX, barY, barW, 3f, 1.5f, ColorProvider.rgba(41, 41, 43, alpha));
        if (hpW > 0.5f) {
            DrawUtil.drawRound(barX, barY, hpW, 3f, 1.5f, ColorProvider.setAlpha(theme, alpha));
        }
        String hpText = String.format(java.util.Locale.US, "%.1f", health);
        DrawUtil.drawText(Fonts.SFREGULAR.get(), hpText, barX, y + 23f, ColorProvider.rgba(255, 255, 255, alpha), 6f);

        // ── Броня (шлем → ботинки) + оффхенд ──
        ItemStack[] armorSlots = new ItemStack[4];
        int armorIndex = 0;
        for (ItemStack stack : living.getArmorItems()) {
            if (armorIndex < 4) armorSlots[armorIndex++] = stack;
        ...
        for (int i = 3; i >= 0; i--) {
            ...
            if (stack != null && !stack.isEmpty()) {
                ...
                if (stack.isDamageable() && stack.getMaxDamage() > 0) {
                    String durability = String.valueOf(stack.getMaxDamage() - stack.getDamage());
                    float durWidth = Fonts.SFMEDIUM.get().getWidth(durability, 4.5f);
                    DrawUtil.drawText(Fonts.SFMEDIUM.get(), durability, iconX + 8f - durWidth / 2f, iconY + 10f,
                            ColorProvider.rgba(255, 255, 255, alpha), 4.5f);
                }
            }
            iconX += 17f;
        }
            ...
            if (!items.isEmpty()) {
                ...
                for (ItemStack stack : items) {
                    ...
                    if (stack.isDamageable() && stack.getMaxDamage() > 0) {
                        ...
                        context.drawText(mc.textRenderer, durability + "%", 0, 10, 0xFFFFFFFF, true);
                    }
                    context.getMatrices().pop();
                    itemX += slotSize + padding;
                }
