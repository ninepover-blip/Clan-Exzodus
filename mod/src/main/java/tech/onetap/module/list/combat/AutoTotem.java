...
public class AutoTotem extends Module {
    ...
    private void updateSwap() {
        ...
        int totemSlot = searchTotem();
        boolean plainAvailable = searchTotem(false) != -1;
        boolean offHandIsEnchantedTotem = offHand.getItem() == Items.TOTEM_OF_UNDYING && offHand.hasEnchantments();

        if (condition()
                && totemStopWatch.isReached(300)
                && totemSlot != -1
                && (offHand.getItem() != Items.TOTEM_OF_UNDYING
                || (offHandIsEnchantedTotem && plainAvailable))) {
    ...
    private int searchTotem() {
        int plain = searchTotem(false);
        if (plain != -1) return plain;
        return searchTotem(true);
    }

    private int searchTotem(boolean enchanted) {
        return InventoryUtil.searchItemStack(stack ->
                stack.getItem() == Items.TOTEM_OF_UNDYING && stack.hasEnchantments() == enchanted);
    }
