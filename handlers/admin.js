import {json,method,sql,requireActor,newLicenseKey,sha256,validSoftware} from '../lib/core.js';
export default async function handler(req,res){
  ...
  if(b.action==='create'){
    ...
    await sql`insert into licenses(key_hash,key_cipher,key_hint,duration_days,software) values(${sha256(key)},${key},${key.slice(-6)},${days},${software})`;
    return json(res,200,{key,software});
  }
  if(b.action==='list'){
    const rows=await sql`select key_cipher as key,key_hint as hint,duration_days as "durationDays",activated_at as "activatedAt",expires_at as "expiresAt",software,(hwid_hash is not null) as "hwidBound",revoked from licenses order by created_at desc limit 500`;
