import {json,method,sql,sha256,validSoftware} from '../lib/core.js';

export default async function handler(req,res){
  if(!method(req,res))return;
  const key=String(req.body?.key||'').trim().toUpperCase(),hwid=String(req.body?.hwid||'').trim();
  const software=String(req.body?.software||'infinity');
  if(!validSoftware(software))return json(res,400,{valid:false,message:'Неизвестный софт'});
  ...
  // У каждого софта — свои ключи. Legacy-ключи без софта работают только для Infinity.
  if(lic.software!=null && lic.software!==software)return json(res,403,{valid:false,message:'Ключ не подходит для выбранного софта'});
  ...
  return json(res,200,{valid:true,expiresAt:expires,maxActivations:Number(lic.max_activations||1),software:lic.software||'infinity'});
}
